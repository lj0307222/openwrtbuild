...
rm -rf ./tmp/info/.packageinfo-feeds_luci_luci-app-alist
...
export Op_name="LJrouter"                # 修改主机名称为OpenWrt-123(填0为不作修改)

# 内核和系统分区大小(不是每个机型都可用)
export Kernel_partition_size="30"            # 内核分区大小,每个机型默认值不一样 (填写您想要的数值,默认一般16,数值以MB计算，填0为不作修改),如果你不懂就填0
...
export Customized_Information="LJ"  # 个性签名,你想写啥就写啥，(填0为不作修改)
